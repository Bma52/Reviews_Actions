...
def create_triplets(df, df_review, df_product, i):
    ...
    if df.iloc[i]["Actions"] == "No_ActionAction":
       ...
       list_subjects = [schema + str(df.iloc[i]['Actions']), 
                 arec + str(df.iloc[i]['Features']), 
                 arec + str(df.iloc[i]['Ability']), 
                 schema + str(df.iloc[i]['Environment']),
                 schema + str(df.iloc[i]['Object']),
                 arec + str(df.iloc[i]['Agent']),
                 arec + str(df.iloc[i]['Ability']),
                 schema + str(df.iloc[i]['Actions']),
                 schema + str(df.iloc[i]['Actions']),
                 schema + str(df.iloc[i]['Actions']),
                 oa + str(df.iloc[i]['annotation_md5']),
                 arec + str(df.iloc[i]['Features']),
                 oa + str(df.iloc[i]['annotation_md5']),
       ...
       list_objects = ['<' + oa + str(df.iloc[i]['annotation_md5']) + '>',
                '<' + oa + str(df.iloc[i]['annotation_md5'])+ '>',
                '<' + oa + str(df.iloc[i]['annotation_md5'])+ '>',
                '<' + oa + str(df.iloc[i]['annotation_md5'])+ '>',
                '<' + oa + str(df.iloc[i]['annotation_md5'])+ '>',
                ...
                '<' + schema + str(df_review.iloc[0]['reviewBody_md5'])+ '>',
                '<' + schema + str(df.iloc[i]['Actions'])+ '>',
                "'" + str(df.iloc[i]['Valence'])+ "'" + '^^'+'<{0}string>'.format(xsd),
                "'" +str(df.iloc[i]['reviewBody'])+"'"  + '^^'+'<{0}string>'.format(xsd),
                '<' + oa + str(df.iloc[i]['annotation_md5'])+ '>',
                '<' + schema + str(df.iloc[i]["Object"])+ '>',
                '<' + schema + str(df.iloc[i]['Actions'])+ '>',
                '<' + arec + str(df.iloc[i]['Features'])+ '>',
                ...
                '<' + schema + str(df_product["product_name_md5"][0])+ '>',
                "'" +str(df.iloc[i]["checkedTimestamp"])+ "'" + '^^'+'<{0}dateTime>'.format(xsd),
                "'" +str(df.iloc[i]['annotation'])+ "'" + '^^'+'<{0}string>'.format(xsd),
...
def main():
     ...
     if checked_by == "At least 2 annotators":
        ...
        checked_data = checked_annotation_data.groupby("annotation_md5").filter(lambda x: len(x) > 1)
        ...
        checked_data = checked_annotation_data.groupby("annotation_md5").filter(lambda x: len(x) > 2)
        #reviews = checked_data["annotation"].unique()
        count = 2
     elif checked_by == "":
        ...
        for review in reviews:
           ...
           checked_data = checked_annotation_data[checked_annotation_data["annotation_md5"].map(checked_annotation_data["annotation"].value_counts()) > count]
