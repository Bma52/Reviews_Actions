...
def create_triplets(df, df_review, df_product, i):
    ...
    if df["Actions"][i] == "No_ActionAction":
       ...
       list_objects = ['<<' + oa + str(df['annotation_md5'][i]) + '>>',
                ...
                '<<' + schema + str(df_review.iloc[0]['reviewBody_md5'])+ '>>',
                '<<' + schema + str(df['Actions'][i])+ '>>',
                re.escape(str(df['Valence'][i])) + '^^'+'<<{0}string>>'.format(xsd),
                re.escape(str(df['reviewBody'][i])) + '^^'+'<<{0}string>>'.format(xsd),
                '<<' + oa + str(df['annotation_md5'][i])+ '>>',
                '<<' + schema + str(df["Object"][i])+ '>>',
                '<<' + schema + str(df['Actions'][i])+ '>>',
                '<<' + arec + str(df['Features'][i])+ '>>',
                '<<' + schema + str(df_product.iloc[0]["product_name_md5"])+ '>>',
                re.escape(str(df_product.iloc[0]["ratingValue"])) + '^^'+'<<{0}decimal>>'.format(xsd),
                '<<' + schema + str(df_product.iloc[0]["seller_name"])+ '>>',
                '<<' + schema + str(df_product.iloc[0]["product_name_md5"])+ '>>',
                re.escape(str(df["checkedTimestamp"][i])) + '^^'+'<<{0}string>>'.format(xsd),
                re.escape(str(df['annotation'][i])) + '^^'+'<<{0}string>>'.format(xsd),
                re.escape(str(df_review.iloc[0]["product_name"])) + '^^'+'<<{0}string>>'.format(xsd),
                re.escape(str(df_review.iloc[0]["product_name"])) + '^^'+'<<{0}string>>'.format(xsd),
                '<<' + schema + str(df_product.iloc[0]["seller_name"])+ '>>',
                re.escape(str(df_product.iloc[0]["availability"])) + '^^'+'<<{0}string>>'.format(xsd),
                re.escape(str(df_product.iloc[0]["price"])) + '^^'+'<<{0}decimal>>'.format(xsd),
                re.escape(str(df_product.iloc[0]["priceCurrency"])) + '^^'+'<<{0}string>>'.format(xsd),
                re.escape(str(df_product.iloc[0]["model"])) + '^^'+'<<{0}string>>'.format(xsd),
                re.escape(str(df_product.iloc[0]["product_name"])) + '^^'+'<<{0}string>>'.format(xsd),
                re.escape(str(df_product.iloc[0]["product_name"])) + '^^'+'<<{0}string>>'.format(xsd),
                re.escape(str(df_product.iloc[0]["description"])) + '^^'+'<<{0}string>>'.format(xsd),
                re.escape(str(df_product.iloc[0]["brand_name"])) + '^^'+'<<{0}string>>'.format(xsd),
