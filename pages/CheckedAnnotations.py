...
def create_triplets(df, df_review, df_product, i):
    ...
    if df["Actions"][i] == "No_ActionAction":
       ...
       list_objects = ['<' + oa + str(df['annotation_md5'][i]) + '>',
                ...
                str(df_review["product_name"][0]) + '^^'+'<{0}string>'.format(xsd),
                str(df_review["product_name"][0]) + '^^'+'<{0}string>'.format(xsd),
                '<' + schema + str(df_product["seller_name"][0])+ '>',
                str(df_product["availability"][0]) + '^^'+'<{0}string>'.format(xsd),
                ...
                str(df_product["url"][0]) + '^^'+'<{0}string>'.format(xsd),
                str(df_product["image"][0]) + '^^'+'<{0}string>'.format(xsd),
               ]    
